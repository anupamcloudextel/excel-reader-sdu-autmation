...
Output columns:
Same as RSU Master update.csv, but with updated:
...
def olt_to_rsu_code(olt_id: str) -> str:
    """Map OLT ID -> RSU Code by appending -0 (after removing spaces)."""
    base = _normalize_no_spaces(olt_id)
...
def build_po_lookup(route_df: pd.DataFrame) -> dict[tuple[str, str], float | str]:
    ...
    """
    df = route_df.copy()
    df["RSU Code"] = df["OLT ID"].astype(str).map(olt_to_rsu_code)
