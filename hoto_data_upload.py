...
def _int_safely(val) -> int:
    """
    Convert Excel numeric-like values to int safely.

    Handles cases like non-breaking spaces '\\xa0', empty strings, and floats
    coming from Excel. Returns 0 when the value is blank or cannot be parsed.
    """
    if pd.isna(val):
        return 0
    ...
    if s == "":
        return 0
    try:
        # Allow for values that are floats but represent whole numbers
        return int(float(s))
    except Exception:
        return 0
    ...
    for (_, hoto_date), group in grouped:
        ...
        for idx, row in group.reset_index(drop=True).iterrows():
            ...
            r = {
                ...
                "ID (Executed)": "",
                "Executed FAT (Executed)": _int_safely(actual_fat),
                "Executed HP (Executed)": _int_safely(actual_hp),
                "Hoto Item (Executed)": hoto_item,
                "Incremental Hoto(fibre length) (Executed)": _int_safely(fibre_length),
            }
            rows.append(r)
